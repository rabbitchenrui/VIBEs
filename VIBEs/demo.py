# -*- coding: utf-8 -*-
# Add the post-optimization( Smoothnet ) to VIBE.
...
#os.environ['EGL_LIBRARY'] = 'D:\\anaconda\envs\\vibe_smooth\Lib\site-packages\OpenGL\platform'
...
from optimize_smoothnet import optimize
MIN_NUM_FRAMES = 25
...
def main(args):
    ...
    for person_id in tqdm(list(tracking_results.keys())):
        ...
        frames_smooth = np.expand_dims(frames, axis=0)
        pred_pose_smooth = np.expand_dims(pred_pose, axis=0)
        pred_betas_smooth = np.expand_dims(pred_betas, axis=0)
        np.savez('example.npz', imgname=frames_smooth, pose=pred_pose_smooth, shape=pred_betas_smooth)

        if args.use_smoothnet:
            pred_pose, pred_betas = optimize(frames_smooth, pred_pose_smooth, pred_betas_smooth)

        pred_betas = pred_betas1
        pred_verts, pred_pose, pred_joints3d = smooth_pose(args, pred_pose, pred_betas,
    ...
    if args.use_smoothnet:
        print(f'VIBE_Smooth FPS: {fps:.2f}')
    else:
    ...
    if not args.no_render:
        ...
        if args.use_smoothnet:
            save_name = f'{vid_name.replace(".mp4", "")}_vibe_smooth_result.mp4'
    ...
    print("all time cost :", all_end_time - all_start_time )
    print('================= END =================')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--vid_file', type=str, default="./2.mp4",
    ...
    parser.add_argument('--vibe_batch_size', type=int, default=150,
    ...
    parser.add_argument('--use_smoothnet', action='store_true',help='use smoothnet')
